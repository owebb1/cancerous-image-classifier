...
def main(argv):
    ...
    c = 0.1
    svc = svm.LinearSVC(
        dual=False, class_weight="balanced", C=c
    )  # , kernel=kernal)

    model = BaggingClassifier(base_estimator=svc, n_estimators=8, n_jobs=-1)
    ...
    if LOAD_SMALL_SET:
        filename = (
            f"/scratch/owebb1/models/bagging_svm_small_{SIZE}_{FILE_NUM}.pkl"
        )
    else:
        filename = f"/scratch/owebb1/models/bagging_svm_{SIZE}_{FILE_NUM}.pkl"

    pickle.dump(model, open(filename, "wb"))
    # print(f"Model dumped into {filename}")
    print(f"{SIZE},{round(end-start,3)}")
